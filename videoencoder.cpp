...
bool VideoEncoder::CreateFile(QString fileName,unsigned width,unsigned height,unsigned bitrate,unsigned gop)
{
	...
	if(!isSizeValid())
	{
		m_err_msg = "Invalid Size\n";
		return false;
	}

	pOutputFormat = ffmpeg::guess_format(NULL, fileName.toStdString().c_str(), NULL);
	if (!pOutputFormat) {
		m_err_msg = "Could not deduce output format from file extension: using MPEG.\n";
	...
	if(!pFormatCtx)
	{
		m_err_msg = "Error allocating format context\n";
		return false;
	}
